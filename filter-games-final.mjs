#!/usr/bin/env node
/**
 * filter-games-final.mjs
 * 
 * Final, strict filtering for actual games of specific types:
 ...
 * 
 * Usage:
 *   node filter-games-final.mjs --input out/permissive_games.json --output out/final_games.json
 */
...
const outputFile = args.output || 'out/final_games.json';
const minStars = parseInt(args['min-stars'] || '5');
const maxResults = parseInt(args['max-results'] || '500');

console.log(`Final filtering for actual games from: ${inputFile}`);
console.log(`Output to: ${outputFile}`);
console.log(`Min stars: ${minStars}`);
console.log(`Max results: ${maxResults}`);
console.log('');

// Keywords that STRONGLY indicate this is a game (not a library/utility)
const STRONG_GAME_KEYWORDS = [
  // FPS games
  'fps game', 'first person shooter game', 'shooter game', 'gun game', 'weapon game',
  
  // Platformers
  'platformer game', 'platform game', 'jumping game', 'runner game', 'side-scroller game',
  
  // Maze games
  'maze game', 'labyrinth game', 'puzzle game', 'escape room game',
  
  // Strategy games
  'strategy game', 'tactical game', 'turn-based game', 'rts game', 'real-time strategy game',
  
  // Digging games
  'digging game', 'mining game', 'excavation game', 'drilling game', 'tunnel game',
  
  // Multiplayer games
  'multiplayer game', 'multi-player game', 'online game', 'coop game', 'cooperative game',
  
  // Soccer games
  'soccer game', 'football game', 'fifa game', 'kicking game',
  
  // Driving games
  'driving game', 'racing game', 'car game', 'vehicle game', 'speed game', 'track game',
  
  // Tower defense
  'tower defense game', 'tower-defense game', 'defense game', 'tower game',
  
  // Escape games
  'escape game', 'escape room game', 'breakout game', 'flee game',
  
  // Clicker games
  'clicker game', 'clicking game', 'idle game', 'incremental game', 'tap game'
];

// Keywords that indicate this is NOT a game (more comprehensive)
const NOT_GAME_KEYWORDS = [
  ...
  'chart', 'graph', 'data', 'analytics', 'visualization', 'diagram',
  'terminal', 'console', 'shell', 'pty', 'tty', 'ssh', 'xterm'
];
...
const SPECIFIC_GAMES = [
  ...
  // Maze/Puzzle
  'pac-man', 'pacman', 'maze runner', 'labyrinth', 'escape room', '2048',
...
function isActuallyAGame(repo) {
  const text = `${repo.name} ${repo.description || ''} ${(repo.topics || []).join(' ')}`.toLowerCase();
  
  // Check for specific game names first
  const hasSpecificGame = SPECIFIC_GAMES.some(game => 
  ...
  // Check if it has strong game keywords (must include "game" in the keyword)
  const hasStrongGameKeywords = STRONG_GAME_KEYWORDS.some(keyword => 
  ...
  // Must have strong game keywords AND not be a utility
  return hasStrongGameKeywords && isNotUtility;
}
...
function getGameType(repo) {
  ...
  if (text.includes('maze') || text.includes('labyrinth') || text.includes('puzzle') || text.includes('2048')) {
    return 'maze';
  }
  if (text.includes('strategy') || text.includes('tactical') || text.includes('turn-based') || text.includes('rts') || text.includes('chess')) {
    return 'strategy';
  }
...
async function main() {
  try {
    ...
    console.log('Final filtering for actual games...');
    const games = [];
    let processed = 0;
    ...
    for (const repo of data) {
      ...
      // Check if it's actually a game
      if (isActuallyAGame(repo)) {
    ...
    console.log(`Found ${games.length} actual games`);
    ...
    console.log(`✅ Done! Found ${games.length} actual games, saved top ${limitedGames.length} to:`);
